...
////DTHANH ADDED
//u64 total_all_exectuting_time_0 = 0;
////DTHANH ADDED
...
u64 linsched_print_task_stats(void)
{
	int i;
	long total_time = 0;
	for (i = 1; i < num_tasks; i++) {
...
static void sleep_run_handle(struct task_struct *p, void *data)
{
  ...
  //     task_thread_info(current)->id,
  //     current->se.sum_exec_runtime);
  //printf("busy %u nsec_per_sec %llu \n",d->busy, d->busy * NSEC_PER_MSEC);
...
//DTHANH ADDED
static void sleep_run_deep_handle(struct task_struct *p, void *data)
{
	struct sleep_run_task *d = data;

  //DTHANH ADDED
  d->wait = p->sched_info.run_delay - d->lwait ; 
  d->lwait = p->sched_info.run_delay; 
  //printf("busy %u nsec_per_sec %llu \n",d->busy, d->busy * NSEC_PER_MSEC);
  //DTHANH ADDED
  //printf("CPU %d / t = %u: Task 0x%d scheduled, Runtime %llu, waited %llu, last wait %llu\n",
  //     smp_processor_id(), (unsigned int)jiffies,
  //     task_thread_info(current)->id,
  //     current->se.sum_exec_runtime, d->wait, d->lwait);
...
struct task_data *linsched_create_deep_task_sleep_run(int sleep, int busy)
{
	...
	td->handle_task = sleep_run_deep_handle;
	return td;
}
//DTHANH ADDED
